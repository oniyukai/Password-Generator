...
def generate_password_cmd(): #輸出密碼並計算總長度
    ...
    characters = [numbers_chkbtns_vars[i].get() + lowercase_chkbtns_vars[i].get() + uppercase_chkbtns_vars[i].get() + 
                  symbols_chkbtns_vars[i].get() + additions_entrys[i].get() for i in range(sections_scale.get())]
...
def clear_settings_cmd(): #歸零所有選項
    ...
    for i in range(max_sections):
        ...
        numbers_chkbtns[i].deselect()
        lowercase_chkbtns[i].deselect()
        uppercase_chkbtns[i].deselect()
        symbols_chkbtns[i].deselect()
        additions_entrys[i].delete(0, 'end')
...
def about_cmd(): #關閉'About'功能，開啟'about_window'視窗
    ...
    tk.Label(about_window, text='Password Generator GUI\nVersion 4.0.pre_23.08.10', justify='left').place(relx=0.2, rely=0.1)
...
def sections_scale_cmd(i=...): #開關輸入內容
    ...
    for i in range(max_sections-1, sections_scale.get()-1, -1):
        ...
        numbers_chkbtns[i].config(state='disabled')
        lowercase_chkbtns[i].config(state='disabled')
        uppercase_chkbtns[i].config(state='disabled')
        symbols_chkbtns[i].config(state='disabled')
        additions_entrys[i].config(state='disabled')
    for i in range(1, sections_scale.get()):
        length_spinboxs[i].config(state='normal')
        numbers_chkbtns[i].config(state='normal')
        lowercase_chkbtns[i].config(state='normal')
        uppercase_chkbtns[i].config(state='normal')
        symbols_chkbtns[i].config(state='normal')
        additions_entrys[i].config(state='normal')
...
#主視窗設定
window = tk.Tk()
window.title('Password Generator GUI v4.0.pre_23.08.10')
...
numbers_chkbtns_vars = []
numbers_chkbtns = []
lowercase_chkbtns_vars = []
lowercase_chkbtns = []
uppercase_chkbtns_vars = []
uppercase_chkbtns = []
symbols_chkbtns_vars = []
symbols_chkbtns = []
additions_entrys = []
for i in range(max_sections):
    numbers_chkbtns_vars.append(tk.StringVar())
    numbers_chkbtns.append(tk.Checkbutton(include_labelframe, bd=0, activeforeground='dodgerblue', 
                                          onvalue='0123456789', offvalue='', variable=numbers_chkbtns_vars[i]))
    numbers_chkbtns[i].grid(row=0, column=i+1, padx=9, sticky=tk.E)
    lowercase_chkbtns_vars.append(tk.StringVar())
    lowercase_chkbtns.append(tk.Checkbutton(include_labelframe, bd=0, activeforeground='dodgerblue', 
                                            onvalue='abcdefghijklmnopqrstuvwxyz', offvalue='', variable=lowercase_chkbtns_vars[i]))
    lowercase_chkbtns[i].grid(row=1, column=i+1, padx=9, sticky=tk.E)
    uppercase_chkbtns_vars.append(tk.StringVar())
    uppercase_chkbtns.append(tk.Checkbutton(include_labelframe, bd=0, activeforeground='dodgerblue', 
                                            onvalue='ABCDEFGHIJKLMNOPQRSTUVWXYZ', offvalue='', variable=uppercase_chkbtns_vars[i]))
    uppercase_chkbtns[i].grid(row=2, column=i+1, padx=9, sticky=tk.E)
    symbols_chkbtns_vars.append(tk.StringVar())
    symbols_chkbtns.append(tk.Checkbutton(include_labelframe, bd=0, activeforeground='dodgerblue', 
                                          onvalue='!@#$%^&*', offvalue='', variable=symbols_chkbtns_vars[i]))
    symbols_chkbtns[i].grid(row=3, column=i+1, padx=9, sticky=tk.E)
    additions_entrys.append(tk.Entry(include_labelframe, width=5, relief='flat'))
    additions_entrys[i].grid(row=4, column=i+1, padx=5)
